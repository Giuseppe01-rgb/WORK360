...
export default function WorkerDashboard() {
    ...
    const loadDailyReports = async (siteId) => {
        ...
        try {
            ...
            // Filter to only today's reports
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const allReports = response.data || [];
            const todayOnly = allReports.filter(r => new Date(r.createdAt) >= today);
            setDailyReports(todayOnly);
        } catch (error) {
                        ...
                        <div className="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-sm">
                            ...
                            {dailyReports.length > 0 && (
                                <div className="mt-8 border-t border-slate-100 pt-6">
                                    <h4 className="font-semibold text-slate-900 text-sm uppercase tracking-wider mb-4">I tuoi report di oggi</h4>
